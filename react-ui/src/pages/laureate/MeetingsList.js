...
function MeetingsList({account}){
    ...
    return(
        <div className="container">
            ...
            <div className="p-2 m-2   align-self-center">
                {meeting.map((meet) => meet.fkLaureateAccountId===account.accountId? (
                    <div className="card p-5 d-flex flex-row m-2">
                        <div className="col align-self-center">
                            <div >
                                {aaccount.map(acc=> acc.accountId===meet.fkGodfatherAccountId?(
                                    <div>
                                        <h3>{acc.firstname} {acc.lastname}</h3>
                                        <div>
                                            <Time value={meet.beginning} format="HH:mm" />-<Time value={meet.ending} format="HH:mm" />
                                        </div>
                                    </div>
                                ):false)}
                            </div>
                        ...
                        <div className="col align-self-center">
                            <RadioButtons name={meet.fkGodfatherAccountId} meeting={meet} accountid={account.accountId} godfatherId={meet.fkGodfatherAccountId}/></div>
    ...
    return (
        <div className="d-flex flex-row">
            <div className="d-flex flex-row col">
                ...
                <div>
                    {meeting.laureateRating==null ? (
                        <Button className=" align-self-center col"  variant="btn btn-success " onClick={()=>handleCall(godfatherId,accountid,note)}>Envoyer</Button>
                    ) : (
                        <div>
                            <Button className=" align-self-center col"  variant="btn btn-success " disabled>Envoyer</Button>
                            <div className="text-success align-self-center mt-3">envoyé</div>
                        </div>
                    )}
                </div>
            </div>
