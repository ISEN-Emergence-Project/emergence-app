        ...
        <div className="container">
            ...
            <div className="p-2 m-2 align-self-center">{meeting.map( meet=> meet.fkGodfatherAccountId===account.accountId?
                <div className="card p-5 d-flex flex-row m-2">
                    ...
                    <div className="col align-self-center"><div>{account.map(acc=> acc.accountId===meet.fkLaureateAccountId?<div><h3>{acc.firstname} {acc.lastname}</h3><div><Time value={meet.beginning} format="HH:mm" />-<Time value={meet.ending} format="HH:mm" /></div></div>:false)}</div></div>
        ...
        <div className="d-flex flex-row ">
            ...
            <div>
                {meeting.godfatherRating==null ? (
                    <Button className=" align-self-center col "  variant="btn btn-success " onClick={()=>handleCall(accountid,laureateId,note)}>Envoyer</Button>
                ) : (
                    <div>
